...
app.post("/api/regions", (req: Request, res: Response) => {
  try {
    ...
    if (!Array.isArray(storeData.regions)) {
      storeData.regions = [];
    }

    // Add new region
    storeData.regions.push(newRegion);

    // Save back to file
    if (writeStoreData(storeData)) {
      console.log("Region added successfully to store.json:", newRegion.name);
      res.status(201).json({
        success: true,
